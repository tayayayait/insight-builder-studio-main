 ...
 * 테스트 데이터 파일 생성 스크립트
 * 
 * 실행: node test-data/generate-test-files.js
 */
...
function generateSatisfactionSurvey() {
    ...
    XLSX.writeFile(wb, path.join(outputDir, 'survey_satisfaction_data.xlsx'));
...
function generateIPASurvey() {
    ...
    XLSX.writeFile(wb, path.join(outputDir, 'survey_ipa_data.xlsx'));
...
function generateSmallTestData() {
    ...
    XLSX.writeFile(wb, path.join(outputDir, 'survey_test_small.xlsx'));
